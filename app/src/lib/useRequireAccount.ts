 ...
 * mode, no email), it prompts sign-in and returns false; otherwise returns true.
 * In mock mode there's no real auth, so it always allows.
 */
export function useRequireAccount() {
  ...
  return useCallback(
    (reason = 'Sign in or create an account to contribute to Kick It.'): boolean => {
      ...
      Alert.alert('Create an account', reason, [
